...
export const encryptData = (data: NodeRSA.Data, publicKey: string) => {
  const key = new NodeRSA(publicKey);
  // key.setOptions({ encryptionScheme: 'pkcs1' });
  return key.encrypt(data, 'base64');
};

export const decryptData = (encryptedData: string | Buffer, privateKey: string | Buffer) => {
  console.log('decryptData privateKey', privateKey);
